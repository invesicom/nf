...
class BrightDataScraperServiceTest extends TestCase
{
    ...
    public function it_transforms_brightdata_review_format_correctly()
    {
        ...
        $this->assertEquals('This is a fantastic product that exceeded my expectations.', $review['review_text']);
        ...
        $this->assertFalse($review['vine_review']);
        $this->assertEquals('United States', $review['country']);
        $this->assertEquals('Verified Purchase', $review['badge']);
        $this->assertEquals('ATEST123', $review['author_id']);
        $this->assertEquals('TestBrand', $review['brand']);
